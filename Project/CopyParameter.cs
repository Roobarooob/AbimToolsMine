        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
          ...
          win.SelectedCategories.Select(c => c.Id).ToList());
            ...
            if (win.NestedMode)
                ...
                return FillFloorParam.ExecuteForElements(doc, allElements, win.FromParam);
            else
                ...
                foreach (var parent in allElements)
                {
                    ...
                    if (IsDuctElement(parent) || IsPipeElement(parent))
                    {
                        foreach (ElementId depId in parent.GetDependentElements(null))
                        {
                            ...
                            if (!(dep is DuctInsulation) && !(dep is PipeInsulation)) continue;

                            Parameter tp = dep.LookupParameter(paramName);
                            if (tp == null || tp.IsReadOnly) { skipped++; continue; }
                            if (onlyEmpty && !IsEmpty(tp)) { skipped++; continue; }
                            if (CopyValue(fp, tp)) copied++; else skipped++;
                            anythingProcessed = true;
